...
APP_NAME = "MinerU PDF 转 Markdown"
APP_VERSION = "2.0.0"
...
def cli_paths() -> tuple[Path, Path]:
    root = project_root()
    return root / "runtime" / "env" / "python.exe", root / "src" / "mineru_cli.py"


def validate_cli() -> None:
    python, cli = cli_paths()
    core = project_root() / "src" / "mineru_core.py"
    missing = [str(path) for path in (python, cli, core) if not path.is_file()]
    if missing:
        raise RuntimeError("MinerU CLI 不完整：\n" + "\n".join(f"- {path}" for path in missing))
...
class MinerUApp:
    ...
    def _build_styles(self) -> None:
        ...
        style.configure(
            "MinerU.Horizontal.TProgressbar",
            troughcolor=SURFACE,
            background=ACCENT,
            bordercolor=SURFACE,
            lightcolor=ACCENT,
    ...
    def _build_ui(self) -> None:
        ...
        self.progress = ttk.Progressbar(content, mode="indeterminate", style="MinerU.Horizontal.TProgressbar")
