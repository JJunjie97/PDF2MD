...
from mineru_core import CORE_VERSION, ConversionOptions, RunResult, run_conversion
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "示例：\n"
            "  mineru-cli paper.pdf\n"
            "  mineru-cli paper.pdf --pages 3-8\n"
            "  mineru-cli paper.pdf --pages 1-3,8,12-15 --profile fast --json\n\n"
            "默认在 PDF 同级创建 <文件名>.mineru，公开结果只有 Markdown、images 和 raw。"
        ),
    )
    parser.add_argument("pdf", help="输入 PDF 文件")
    parser.add_argument("-o", "--output", help="指定结果目录；默认是 PDF 同级的 <文件名>.mineru")
    ...
    parser.add_argument("--ocr", action="store_true", help="强制 OCR；默认由 MinerU 自动判断")
    ...
    parser.add_argument("--version", action="version", version=f"mineru-cli {CORE_VERSION}")
    return parser
