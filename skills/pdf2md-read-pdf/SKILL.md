...
5. Read only the single top-level Markdown path returned in JSON. Search it before loading large sections. Tables in this public file are normalized to GFM Markdown where safely possible, and public images are numbered by first reference. When the source contents page is present, use its compact numeric links such as `#1` and `#2` to navigate to headings available in this Markdown; PDF page-number text is intentionally omitted, and an unlinked plain entry may simply point outside the selected page set. Do not expand conversion for an unlinked entry alone. Open files in the returned `images_dir` only when the answer depends on a figure or visual verification.
