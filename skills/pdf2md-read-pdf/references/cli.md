...
```powershell
scripts\pdf2md-pdf.cmd inspect "D:\docs\paper.pdf"
scripts\pdf2md-pdf.cmd search "D:\docs\paper.pdf" --query "power supply current" --top-k 8
scripts\pdf2md-pdf.cmd prepare "D:\docs\paper.pdf" --query "What is the maximum input voltage?"
scripts\pdf2md-pdf.cmd convert "D:\docs\paper.pdf" --pages "3-8" --profile balanced
scripts\pdf2md-pdf.cmd status "D:\docs\paper.pdf"
```

All commands emit one UTF-8 JSON object on stdout. `prepare` and `convert` return one `markdown` path and one `images_dir`. Read those only. Internal diagnostic output is stored under `<pdf-stem>.pdf2md/raw` and may be written to stderr.
...
- `inspect`: Return page count, PDF kind, outline entries, contents-page candidates, sampled character counts, and estimated tokens without starting the OCR engine.
...
- `--timeout N`: PDF2MD timeout in seconds; default `1800`.
