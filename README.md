...
- 顶层 `images/` 只发布 Markdown 实际引用的图、照片、结构图等视觉内容。
- OCR 过程中生成但 Markdown 未引用的公式裁剪图、版面切块等只保存在 `raw/cache/images/`，不会污染公开图片目录。
- 转换层会严格检查 UTF-8 和 Unicode 替换字符 `�`。`auto` 使用 PDF 文本层后若发现损坏字符，会对同一页段自动强制视觉 OCR；旧缓存中含 `�` 时会直接失效并转为 OCR，不再发布已知损坏的 Markdown。
- 如果强制视觉 OCR 后仍存在 `�`，任务会明确失败并保留原有公开结果，不会静默覆盖成品。状态栏会显示受影响页段和字符数量。
...
8. **缺陷感知回退**：正常文档保留 `auto` 的速度；只有检测到 `�` 的页段才增加一次视觉 OCR。已知损坏缓存直接走 OCR，避免重复运行必然失败的文本层解析。
...
完成安装和构建后，可以把本地根目录从 `minerU` 手动改成 `PDF2MD`。代码、环境和模型都使用相对项目结构；首次运行时还会自动修复 `runtime/pdf2md.json` 中因目录改名而失效的本地模型绝对路径。请先关闭 PDF2MD、终端中的 OCR 子进程及占用项目目录的编辑器任务，再执行改名。
