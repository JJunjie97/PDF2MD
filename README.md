...
- PDF2MD 会识别中文/英文正文目录、图目录、表目录、续页标题及常见书籍目录标题。全文转换时只检查 PDF 前置区，最多 64 个物理页，并在离开目录区后提前停止；高质量原生文本层可以修复 OCR 中的断行、双栏粘连、点线和页码错位。对于文本层不可用的扫描版论文，只有检测到连续编号和页码/点引导符等版式证据时，才会恢复无页码目录与独立页码列。结果缓存于 `raw/cache/frontmatter-v6.json`，不会反复扫描。
- 最终目录只保留名称和层级，不显示原 PDF 目录页码。章、`1.1`、`1.1.1` 按编号深度逐级缩进；标题折行、单空格页码、罗马页码、字距异常和续页会在前置页解析器中保守合并。
- 正文目录只能链接到原文已经存在的 `#` 到 `######` Markdown 标题。普通正文行即使文字相同也不会被提升或作为链接目标；编号条目必须匹配同一编号，`1` 与 `A.1`、章节与图表也不会跨类型匹配。无编号标题只有在全文唯一时才连接，模糊匹配还要求足够置信度和唯一差距。
...
- 局部页转换不会注入完整 PDF 的目录，避免无目标链接与额外 token。无法找到唯一安全目标的条目保留为纯文本，不猜测；没有源目录的文档也不会自动插入长目录。
...
9. **零模型目录整理**：只扫描前置页原生文本并缓存目录结构；目录层级、严格标题匹配、图表索引和双向链接都在本地文本发布阶段处理，复用 OCR 缓存时无需重新推理。
...
│  ├─ pdf2md_frontmatter.py        # 前置页原生目录提取和独立缓存
│  ├─ pdf2md_toc.py                # 目录整理、严格匹配和双向链接
...
│  ├─ audit-navigation.py          # 批量检查目录锚点、双链和幂等性
