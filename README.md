...
- 最终目录只保留章节名称和层级，不再显示原 PDF 目录页码。成功匹配的条目使用标准 Markdown 链接，如 `[1.1 Overview](#2)`；链接目标由正文前的显式 `<a id="2"></a>` 提供，并仅对实际链接到的正文标题按出现顺序连续编号为 `1、2、3…`，不会出现前缀或跳号。这里保留一行最小 HTML，是因为纯 Markdown 没有跨渲染器统一的自定义锚点语法；显式数字锚点对中文、重复标题和不同 Markdown 阅读器更稳定。
